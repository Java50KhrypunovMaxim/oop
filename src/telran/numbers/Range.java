...
public class Range implements Iterable<Integer> {
	...
	private class RangeIterator implements Iterator<Integer> {
		int current = containtsInRemoved(minInclusive) ? getCurrent(minInclusive) : minInclusive;
		int previous = minInclusive;// initial value not important
		...
		private int getCurrent(int current) {
			current ++;
			while (current < maxExclusive && containtsInRemoved(current))
			{
				current ++;
			}
			return current;
		...
		private boolean containtsInRemoved(int number) {
			boolean res = false;
			int index = 0;
	...
	private void addRemove(int number) {
	...
	public boolean removeIf(Predicate<Integer> predicate) {
		...
		while (it.hasNext()) {
			int num = it.next();// сurrent number
			if (predicate.test(num)) // удоволетворяет ли условию предиката или нет
			{
				it.remove();
			}
		}
